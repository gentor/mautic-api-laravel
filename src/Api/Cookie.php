...
use Gentor\Mautic\Cookie as StandardCookie;
...
class Cookie extends StandardCookie
{
    ...
    {
        $this->unsetKey(self::MTC_ID);

        if ($sessionId = $this->getSessionId()) {
            $this->unsetKey($sessionId);
        }
    ...
    {
        $this->unsetKey(self::MAUTIC_SESSION_ID);
        $this->unsetKey(self::MTC_SID);
